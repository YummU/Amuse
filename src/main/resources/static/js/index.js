...
(function (a) {
    ...
    a.supersized = function (b) {
        ...
        d._start = function () {
            ...
            let o = d.options.new_window ? ' target="_blank"' : "";
            ...
            if (d.options.random) {
                ...
                for (let h, m, k = arr.length; k; h = parseInt(Math.random() * k), m = arr[--k], arr[k] = arr[h], arr[h] = m) {
                }
            ...
            if (d.options.slides.length > 1) {
                if (d.options.slides.length > 2) {
                    ...
                    let g = (d.options.slides[loadPrev].url) ? "href='" + d.options.slides[loadPrev].url + "'" : "";
                    let q = a('<img src="' + d.options.slides[loadPrev].image + '"/>');
                    let n = d.el + " li:eq(" + loadPrev + ")";
            ...
            let l = a('<img src="' + api.getField("image") + '"/>');
            let f = d.el + " li:eq(" + vars.current_slide + ")";
        ...
        d.resizeNow = function () {
            return d.$el.each(function () {
                a("img", d.el).each(function () {
                    thisSlide = a(this);
                    let f = (thisSlide.data("origHeight") / thisSlide.data("origWidth")).toFixed(2);
                    let e = d.$el.width(), h = d.$el.height(), i;
        ...
        d.nextSlide = function () {
            ...
            clearInterval(vars.slideshow_interval);
            let h = d.options.slides, e = d.$el.find(".activeslide");
            ...
            let g = a(d.el + " li:eq(" + vars.current_slide + ")"), i = d.$el.find(".prevslide");
            ...
            let k = d.el + " li:eq(" + loadSlide + ")";
            if (!a(k).html()) {
                let j = d.options.new_window ? ' target="_blank"' : "";
                imageLink = (d.options.slides[loadSlide].url) ? "href='" + d.options.slides[loadSlide].url + "'" : "";
                let f = a('<img src="' + d.options.slides[loadSlide].image + '"/>');
        ...
        d._placeSlide = function (e) {
            let h = d.options.new_window ? ' target="_blank"' : "";
            loadSlide = false;
            if (e == "next") {
                vars.current_slide == d.options.slides.length - 1 ? loadSlide = 0 : loadSlide = vars.current_slide + 1;
                let g = d.el + " li:eq(" + loadSlide + ")";
                if (!a(g).html()) {
                    let h = d.options.new_window ? ' target="_blank"' : "";
                    imageLink = (d.options.slides[loadSlide].url) ? "href='" + d.options.slides[loadSlide].url + "'" : "";
                    let f = a('<img src="' + d.options.slides[loadSlide].image + '"/>');
